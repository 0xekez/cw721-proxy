...
fn add_to_whitelist_authorized() {
    ...
    test.add_to_whitelist(test.minter.clone(), 1234).unwrap();
}
...
fn remove_from_whitelist_authorized() {
    let mut test = Test::new(1);
    test.remove_from_whitelist(test.minter.clone(), 1234)
        .unwrap();
}
...
#[test]
fn send_authorized_code_id() {
    let mut test = Test::new(1);
    test.add_to_whitelist(test.minter.clone(), test.cw721_id)
        .unwrap();

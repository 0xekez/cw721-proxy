...
fn remove_from_whitelist_authorized() {
    let mut test = Test::new(1);
    test.remove_from_whitelist(test.minter.clone(), test.cw721s[0].to_string())
        .unwrap();
}
...
#[test]
fn send_authorized_sender_and_channel() {
    let mut test = Test::new(1);
        ...
        cw721_proxy_tester::msg::ExecuteMsg::ReceiveProxyNft { eyeball, msg } => {
            ...
            assert_eq!(
                ...
                cw721::Cw721ReceiveMsg {
                    ...
                    token_id,
                    msg: to_binary(&test.ibc_outgoing_msg(channel_id.to_string())).unwrap(),
                }
            )
        }
    }
...
fn send_unauthorized_sender() {
    let mut test = Test::new(1);
    let token_id = test.mint(test.cw721s[0].clone()).unwrap();
    let ungoverned_channel_id = "ungovernedchannel-123";
    let err: ContractError = test
        .send_nft(
            test.cw721s[0].clone(),
            token_id,
            ungoverned_channel_id.to_string(),
        )
        .unwrap_err()
        .downcast()
        .unwrap();
...
fn send_unauthorized_channel() {
    ...
    let token_id = test.mint(test.cw721s[0].clone()).unwrap();
    let ungoverned_channel_id = "ungoverned";
    let err: ContractError = test
        .send_nft(
            test.cw721s[0].clone(),
            token_id,
    ...
    assert_eq!(
        ...
        ContractError::UnauthorizedChannel {
            channel: ungoverned_channel_id.to_string()
        }
    )
}
